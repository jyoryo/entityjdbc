...
public class ClassPathResource extends UrlResource {
	...
	public ClassPathResource(String pathBaseClassLoader, ClassLoader classLoader, Class<?> clazz) {
		...
		this.path = path;
		this.name = Strings.isBlank(path) ? null : FilenameUtils.getName(path);
		
		this.classLoader = Objects.defaultIfNull(classLoader, ClassUtil.getClassLoader());
		this.clazz = clazz;
		initUrl();
	}
	// -------------------------------------------------------------------------------------- Constructor end
	...
	public final String getAbsolutePath() {
		if (Files.isAbsolutePath(this.path)) {
			return this.path;
		}
		// url在初始化的时候已经断言，此处始终不为null
		return FilenameUtils.normalize(URLs.getDecodedPath(this.url));
	}
	...
	private String normalizePath(String path) {
		...
		path = Strings.removePrefix(path, Strings.SLASH);

		Assert.isFalse(Files.isAbsolutePath(path), "Path [{}] must be a relative path !", path);
		return path;
	}
}
