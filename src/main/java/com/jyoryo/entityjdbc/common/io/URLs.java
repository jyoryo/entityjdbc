...
import com.jyoryo.entityjdbc.common.io.resource.Resources;
import com.jyoryo.entityjdbc.common.utils.Urls;
...
public class URLs {
	...
	public static URL url(String url, URLStreamHandler handler) {
		...
		if (url.startsWith(CLASSPATH_URL_PREFIX)) {
			...
			return ClassUtil.getClassLoader().getResource(url);
		}
	...
	public static URL toUrlForHttp(String urlStr, URLStreamHandler handler) {
		...
		urlStr = Strings.cleanBlank(urlStr);
		return URLs.url(urlStr, handler);
	}
	 ...
	 * @return URL
	 * @see Resources#getClasspathURL(String)
	 */
	public static URL getClasspathURL(String pathBaseClassLoader) {
		return Resources.getClasspathURL(pathBaseClassLoader);
	}
	 ...
	 * @param clazz 指定class
	 * @return URL
	 * @see Resources#getClasspathURL(String, Class)
	 */
	public static URL getClasspathURL(String path, Class<?> clazz) {
		return Resources.getClasspathURL(path, clazz);
	}
	...
	public static String encode(String url, Charset charset) {
		...
		return Urls.encode(url, CharsetUtil.toCharset(charset));
	}
	...
	public static String encode(String url, String charset) throws UtilException {
		...
		return encode(url, CharsetUtil.toCharset(charset));
	}
	...
	public static String encodeQuery(String url, String charset) throws UtilException {
		return encodeQuery(url, CharsetUtil.toCharset(charset));
	}
	...
	public static String decode(String content, Charset charset) {
		return decode(content, CharsetUtil.toCharset(charset).name());
	}
	...
	public static boolean isJarFileURL(URL url) {
		return (URL_PROTOCOL_FILE.equals(url.getProtocol()) && //
			url.getPath().toLowerCase().endsWith(Files.JAR_FILE_EXT));
	}
	...
	public static BufferedReader getReader(URL url, Charset charset) {
		return IOs.getReader(getStream(url), charset);
	}
	...
	public static String normalize(String url, boolean isEncodeBody) {
		...
		String body;
		if(sepIndex > 0) {
			pre = Strings.subPre(url, sepIndex + 3);
			body = Strings.subSuf(url, sepIndex + 3);
		}else {
			pre = "http://";
			body = url;
		}
		
		final int paramsSepIndex = Strings.indexOf(body, '?');
		String params = null;
		if(paramsSepIndex > 0) {
			params = Strings.subSuf(body, paramsSepIndex);
			body = Strings.subPre(body, paramsSepIndex);
		}
		
		//去除开头的\或者/
		body = body.replaceAll("^[\\/]+", Strings.EMPTY);
		//替换多个\或/为单个/
		body = body.replace("\\", "/").replaceAll("//+", "/");
		if(isEncodeBody) {
			body = encode(body);
		}
		return pre + body + Strings.defaultString(params);
	}
