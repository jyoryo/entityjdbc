...
/**
 * 路径监听器，可以监听目录或文件。
 * <li>如果监听的Path不存在，则递归创建该目录然后监听此目录；</li>
 * <li>递归监听目录时，并不会监听新创建的目录</li>
 * @author jyoryo
 *
 */
public class WatchMonitor extends Thread implements Closeable {
    ...
    public static WatchMonitor create(URL url, int maxDepth, WatchEvent.Kind<?>... events){
        return create(URLs.toURI(url), maxDepth, events);
    }
    ...
    public void init() throws WatchException{
        //获取目录或文件路径
        if(false == java.nio.file.Files.exists(this.path, LinkOption.NOFOLLOW_LINKS)) {
            final Path lastPathEle = Files.getLastPathEle(this.path);
            if(null != lastPathEle) {
                final String lastPathEleStr = lastPathEle.toString();
                //带有点表示有扩展名，按照未创建的文件对待。Linux下.d的为目录，排除之
                if(Strings.contains(lastPathEleStr, Chars.DOT) && false == Strings.endsWithIgnoreCase(lastPathEleStr, ".d")) {
        ...
        }else if(java.nio.file.Files.isRegularFile(this.path, LinkOption.NOFOLLOW_LINKS)){
            this.filePath = this.path;
            this.path = this.filePath.getParent();
        }
...
//                  log.debug("[{}] is not fit for [{}], pass it.", event.context(), this.filePath.getFileName());
    ...
    private void registerPath(Path path, int maxDepth) {
        try {
            final WatchKey key = path.register(watchService, Arrays.isEmpty(this.events) ? EVENTS_ALL : this.events);
            watchKeyPathMap.put(key, path);
            if(maxDepth > 1) {
                //遍历所有子目录并加入监听
                java.nio.file.Files.walkFileTree(path, EnumSet.noneOf(FileVisitOption.class), maxDepth, new SimpleFileVisitor<Path>(){
                    @Override
