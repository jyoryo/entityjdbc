...
public class Iterators {
	...
	public static <K, V> Map<K, V> fieldValueMap(Iterator<V> iter, String fieldName) {
		...
		if (null != iter) {
			...
			while (iter.hasNext()) {
				value = iter.next();
				result.put((K) Reflects.readField(fieldName, value), value);
			}
		}
		return result;
	}
	...
	public static <K, V> Map<K, V> fieldValueAsMap(Iterator<?> iter, String fieldNameForKey, String fieldNameForValue) {
		...
		if (null != iter) {
			...
			while (iter.hasNext()) {
				value = iter.next();
				result.put((K) Reflects.readField(fieldNameForKey, value), (V) Reflects.readField(fieldNameForValue, value));
			}
		}
		return result;
	}
	...
	public static <V> List<Object> fieldValueList(Iterator<V> iter, String fieldName) {
		...
		if (null != iter) {
			...
			while (iter.hasNext()) {
				value = iter.next();
				result.add(Reflects.readField(fieldName, value));
			}
		}
		return result;
	}
	...
	public static <T> String join(Iterator<T> iterator, CharSequence conjunction, String prefix, String suffix) {
		...
		while (iterator.hasNext()) {
			...
			if (Arrays.isArray(item)) {
				sb.append(Arrays.join(Arrays.wrap(item), conjunction, prefix, suffix));
			} else if (item instanceof Iterable<?>) {
				sb.append(join((Iterable<?>) item, conjunction, prefix, suffix));
			} else if (item instanceof Iterator<?>) {
				sb.append(join((Iterator<?>) item, conjunction, prefix, suffix));
			} else {
				sb.append(Strings.wrap(String.valueOf(item), prefix, suffix));
			}
		}
		return sb.toString();
	}
	...
	public static <K, V> Map<K, V> toMap(Iterator<K> keys, Iterator<V> values, boolean isOrder) {
		final Map<K, V> resultMap = Maps.newHashMap(isOrder);
		if (isNotEmpty(keys)) {
			while (keys.hasNext()) {
